...
from AeroRobot import telethn as bot
...
async def proboyx(event):
    ...
    on = await bot.send_file(event.chat_id, file=file2,caption= f"♡ Hey {betsy}, I'm Aerodynamic\n♡ I'm Created By [Prince](tg://user?id=1484735126)\n♡ Click The Button Below To Get Your Info", buttons=button)
...
async def callback_query_handler(event):
  try:
    ...
    AERODYNAMIC = "YOUR DETAILS BY AERODYNAMIC \n\n"
    AERODYNAMIC += f"FIRST NAME : {PRO.first_name} \n"
    AERODYNAMIC += f"LAST NAME : {PRO.last_name}\n"
    AERODYNAMIC += f"YOU BOT : {PRO.bot} \n"
    AERODYNAMIC += f"RESTRICTED : {PRO.restricted} \n"
    AERODYNAMIC += f"USER ID : {boy}\n"
    AERODYNAMIC += f"USERNAME : {PRO.username}\n"
    await event.answer(AERODYNAMIC, alert=True)
